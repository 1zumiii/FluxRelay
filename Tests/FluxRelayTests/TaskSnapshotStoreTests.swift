...
final class TaskSnapshotStoreTests: XCTestCase {
  ...
  override func setUp() async throws {
    ...
    config = AppConfig(supportDirectory: directory, rpcPort: 16800, rpcSecret: "",
  ...
  func testDerivedTaskListsFollowTheirInputs() async {
    _ = installTask("active")
    let store = TaskSnapshotStore(config: config, client: client)
    let model = MainWindowModel(config: config, client: client, snapshots: store)
    await store.refresh()

    XCTAssertEqual(model.filteredTasks.map(\.id), ["abc"])
    XCTAssertEqual(model.count(for: .all), 1)
    XCTAssertEqual(model.count(for: .active), 1)
    XCTAssertEqual(model.count(for: .completed), 0)

    model.searchText = "file.zip"
    XCTAssertEqual(model.filteredTasks.count, 1)
    model.searchText = "no-such-task"
    XCTAssertTrue(model.filteredTasks.isEmpty)
    model.searchText = ""

    model.select(.tasks(.completed))
    XCTAssertTrue(model.filteredTasks.isEmpty)
    model.select(.tasks(.active))
    XCTAssertEqual(model.filteredTasks.map(\.id), ["abc"])

    _ = installTask("complete")
    await store.refresh()
    XCTAssertTrue(model.filteredTasks.isEmpty, "The active list must drop a task that finished")
    XCTAssertEqual(model.count(for: .active), 0)
    XCTAssertEqual(model.count(for: .completed), 1)
  }

  func testSearchMatchesEveryIndexedField() async throws {
    _ = installTask("complete")
    let store = TaskSnapshotStore(config: config, client: client)
    store.register("abc", sourceURI: "https://source.test/archive", checksum: "sha-256=abc")
    await store.refresh()
    let task = try XCTUnwrap(store.tasks.first)
    for query in ["source.test", "sha-256", directory.lastPathComponent] {
      XCTAssertTrue(task.matchesSearch(query), query)
      XCTAssertTrue(task.historySearchText.localizedCaseInsensitiveContains(query), query)
    }
    XCTAssertFalse(task.matchesSearch("no-such-field"))
