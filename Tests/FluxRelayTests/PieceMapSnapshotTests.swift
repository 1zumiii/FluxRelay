...
final class PieceMapSnapshotTests: XCTestCase {
  ...
  func testPreparationBenchmark() throws {
    guard ProcessInfo.processInfo.environment["FLUXRELAY_PERFORMANCE_CHECK"] == "1" else {
      throw XCTSkip("Opt in to the legacy/new comparison with FLUXRELAY_PERFORMANCE_CHECK=1")
    }
    let count = 16384
