...
if CommandLine.arguments.contains("--self-check") {
  let appIdentityReady = Bundle.main.bundleIdentifier == "io.github.1zumiii.fluxrelay"
  ...
  let supportReady = config.supportDirectory.lastPathComponent == AppIdentity.displayName
  ...
  let adaptiveProfileLimitReady: Bool = {
    let profileURL = FileManager.default.temporaryDirectory
      .appendingPathComponent("fluxrelay-adaptive-check-\(UUID().uuidString).json")
  ...
  let taskTuningURI = "https://fluxrelay-self-check.invalid/archive.bin"
  ...
  let logRotationReady: Bool = {
    let directory = FileManager.default.temporaryDirectory
      .appendingPathComponent("fluxrelay-log-check-\(UUID().uuidString)", isDirectory: true)
