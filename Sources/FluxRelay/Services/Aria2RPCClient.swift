...
struct Aria2Task: Identifiable {
  ...
  // Configured once and never mutated; Foundation's date formatters are safe to
  // read from several threads. Building one per search was measurable.
  private nonisolated(unsafe) static let completionDateFormatter = ISO8601DateFormatter()
  ...
  var historySearchText: String {
    ([
      ...
      completionDate.map(Self.completionDateFormatter.string(from:)),
      checksum,
  ...
  /// Search without building the joined text: most queries match the name, and a
  /// filter pass runs over every task on every keystroke.
  func matchesSearch(_ query: String) -> Bool {
    ...
    if contains(name) || contains(sourceURI) || contains(directory) || contains(primaryFileURL?.path) {
      return true
    }
    if contains(checksum) || contains(checksumResult.title) || contains(checksumResult.rawValue) {
      return true
    }
    if let completionDate,
       contains(Formatting.date(completionDate)) || contains(Self.completionDateFormatter.string(from: completionDate)) {
      return true
    }
    for file in files {
      if contains(file["path"] as? String) { return true }
      for item in (file["uris"] as? [[String: Any]]) ?? [] where contains(item["uri"] as? String) {
        return true
      }
    }
    return false
...
final class Aria2RPCClient {
  ...
  private(set) var runningConfig: AppConfig
  ...
  func updateDefaults(_ config: AppConfig) {
    self.config = config
  }
