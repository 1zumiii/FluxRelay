...
final class MainWindowModel: ObservableObject {
  ...
  @Published var tasks: [Aria2Task] = [] { didSet { invalidateDerivedTasks() } }
  @Published var globalStat = Aria2GlobalStat(downloadSpeed: 0, uploadSpeed: 0, active: 0, waiting: 0, stopped: 0)
  @Published var filter: Filter = .all
  @Published var selectedSection: Section = .tasks(.all) { didSet { invalidateDerivedTasks() } }
  @Published var searchText = "" { didSet { invalidateDerivedTasks() } }
  @Published var taskSort: TaskSort = .engine { didSet { invalidateDerivedTasks() } }
  ...
  private var cachedFilteredTasks: [Aria2Task]?
  private var cachedCounts: [Filter: Int]?
  private var liveTaskIDs: Set<String> { snapshots.liveTaskIDs }
  private var runningSettings: SettingsDraft
  private let restartEngineAction: () async -> Bool
  private let settingsDidSave: (AppConfig) -> Void
  ...
  init(
    ...
    settingsDidSave: @escaping (AppConfig) -> Void = { _ in }
  ) {
    self.config = config
    self.client = client
  ...
  /// A body pass reads the visible tasks and the sidebar counts several times
  /// each. Both are derived from the same inputs, so they are computed once per
  /// change instead of once per read.
  var filteredTasks: [Aria2Task] {
    if let cachedFilteredTasks { return cachedFilteredTasks }
    let result = computeFilteredTasks()
    cachedFilteredTasks = result
    return result
  }
  ...
  private func computeFilteredTasks() -> [Aria2Task] {
  ...
  func count(for filter: Filter) -> Int {
    if let cachedCounts { return cachedCounts[filter] ?? 0 }
    var counts: [Filter: Int] = [.all: tasks.count, .active: 0, .waiting: 0, .paused: 0, .completed: 0]
    for task in tasks {
      switch task.status {
      case "active": counts[.active, default: 0] += 1
      case "waiting": counts[.waiting, default: 0] += 1
      case "paused": counts[.paused, default: 0] += 1
      case "complete": counts[.completed, default: 0] += 1
      default: break
      }
    }
    cachedCounts = counts
  ...
  func engineDidRestart(_ config: AppConfig) {
    guard !isRestartingEngine else { return }
