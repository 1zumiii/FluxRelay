...
struct AppConfig {
  ...
  static func load() -> AppConfig {
    ...
    let supportDirectory = applicationSupport.appendingPathComponent(AppIdentity.displayName, isDirectory: true)
    let previousSupportDirectory = applicationSupport.appendingPathComponent("Motrix Native", isDirectory: true)
    let legacySupportDirectory = applicationSupport.appendingPathComponent("Motrix", isDirectory: true)

    moveSupportDirectoryIfNeeded(from: previousSupportDirectory, to: supportDirectory)
    ...
    return AppConfig(
      ...
      aria2LogPath: supportDirectory.appendingPathComponent(aria2LogFilename),
      systemConfig: system,
      userConfig: user
    )
  }

  func updating(system: [String: Any]? = nil, user: [String: Any]? = nil) -> AppConfig {
    AppConfig(
      supportDirectory: supportDirectory,
  ...
  private static let aria2LogFilename = "aria2.log"

  // Data written before the FluxRelay rename lives in "Motrix Native"; move it once so
  // settings, the session, history and logs carry over under the new name.
  private static func moveSupportDirectoryIfNeeded(from previousDirectory: URL, to directory: URL) {
    let fileManager = FileManager.default
    guard
      fileManager.fileExists(atPath: previousDirectory.path),
      !fileManager.fileExists(atPath: directory.path),
      (try? fileManager.moveItem(at: previousDirectory, to: directory)) != nil
    else {
      return
    }

    let previousLog = "motrix-native-aria2"
    let contents = (try? fileManager.contentsOfDirectory(atPath: directory.path)) ?? []
    for filename in contents where filename.hasPrefix(previousLog) {
      let renamed = "aria2" + filename.dropFirst(previousLog.count)
